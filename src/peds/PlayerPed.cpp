...
CPlayerPed::ProcessControl(void)
{
	...
	// AI NPC Dialogue - Press T to freeze/unfreeze nearest NPC
	{
		static CPed *pFrozenNpc = nil;
		CPad *pad0 = CPad::GetPad(0);

		// Every frame: hard-freeze the NPC by killing all movement
		if (pFrozenNpc) {
			if (pFrozenNpc->DyingOrDead()) {
				pFrozenNpc = nil;
			} else {
				pFrozenNpc->SetPedState(PED_IDLE);
				pFrozenNpc->SetMoveState(PEDMOVE_STILL);
				pFrozenNpc->m_vecMoveSpeed = CVector(0.0f, 0.0f, 0.0f);
				pFrozenNpc->m_objective = OBJECTIVE_WAIT_ON_FOOT;
				pFrozenNpc->m_objectiveTimer = CTimer::GetTimeInMilliseconds() + 10000;
				// Keep facing the player
				CVector dir = GetPosition() - pFrozenNpc->GetPosition();
				pFrozenNpc->m_fRotationCur = CGeneral::GetRadianAngleBetweenPoints(dir.x, dir.y, 0.0f, 0.0f);
				pFrozenNpc->m_fRotationDest = pFrozenNpc->m_fRotationCur;
			}
		}

		if (pad0->GetCharJustDown('T')) {
			printf("[AI] T pressed! nearPeds=%d\n", m_numNearPeds);
			fflush(stdout);
			if (pFrozenNpc) {
				pFrozenNpc->SetObjective(OBJECTIVE_NONE);
				pFrozenNpc->SetWanderPath(CGeneral::GetRandomNumber() & 7);
				printf("[AI] Unfroze NPC\n");
				fflush(stdout);
				pFrozenNpc = nil;
			} else {
				CPed *nearest = nil;
				float nearestDist = 8.0f;
				for (int i = 0; i < m_numNearPeds; i++) {
					CPed *ped = m_nearPeds[i];
					if (!ped || ped->IsPlayer()) continue;
					if (ped->DyingOrDead()) continue;
					float dist = (ped->GetPosition() - GetPosition()).Magnitude();
					if (dist < nearestDist) {
						nearestDist = dist;
						nearest = ped;
					}
				...
				if (nearest) {
					pFrozenNpc = nearest;
					printf("[AI] Froze NPC dist=%.1f\n", nearestDist);
					fflush(stdout);
				} else {
					printf("[AI] No NPC within range\n");
					fflush(stdout);
				}
			}
		}
